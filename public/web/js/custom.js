$(document).on('click' ,'.formBody' ,function(){
$('body').addClass('modal-open');    
});

$(document).on('click','a.passShow span' ,function(){
    
if($('a.passShow span').hasClass('passwordAll')){
    $('a.passShow span').removeClass('passwordAll');
    $(this).removeClass('fa fa-eye-slash').addClass('fa fa-eye');
//    $('.showAll').attr('type','text');
    $(this).parents('.form-group').find('.showAll').attr('type','text');
}
    else{
       $('a.passShow span').addClass('passwordAll');
        $('a.passShow span').removeClass('fa fa-eyeh').addClass('fa fa-eye-slash');
//         $('.showAll').attr('type','password');
         $(this).parents('.form-group').find('.showAll').attr('type','password');
    }
});

$(document).on('click','.clickBtn2' ,function(Event){
   Event.preventDefault();
   
    var date_val = $(".btnBack.active").text();
    $("#timeSlotModal").modal('hide');
    $(".TimeShow").val(date_val)
});
...
$(document).on('click', '.addressBtn .formBtnnew', function () {
...
$(document).on('click', '.timeAll button', function () {
    ...
    $('.timeAll button').removeClass('active');
...
$(document).on('change', '#datepicker', function(){
    var date = $(this).val();
    $.post($(this).data('url'), {date : date}, function(res){
        if(res){
            $("#timeOptn").val('');
            $(".disable_btn").prop('disabled', true);
            $(".disable_btn").removeClass('btnBack');
        }else{
            $(".disable_btn").prop('disabled', false);
            $(".disable_btn").addClass('btnBack');
        }
    })
})
// Fixed Header Start
...
$(document).ready(function () {
    $(document).on('click', '.addrSection', function () {
...
$(document).ready(function () {
    $(document).on('click', '.addrInner li', function () {
        var elm = $(this).text();
